...
class Qwen(nn.Module): 
  ...
  def forward(self, ids, targets=None, reduction='mean', chunk=False):
    B, T = ids.size()
    ...
    print("Allocated BEFORE:", torch.cuda.memory_allocated('cuda:0'))
    # print("Reserved BEFORE:", torch.cuda.memory_reserved('cuda:0'))
    ...
    if chunk:
        ...
        if targets is not None:
            ...
            for i in range(0, T, chunk_size):
                ...
                print("Allocated LOGITS:", torch.cuda.memory_allocated('cuda:0'))
                # print("Reserved LOGITS:", torch.cuda.memory_reserved('cuda:0')) 
                ...
                print("Allocated LOSS:", torch.cuda.memory_allocated('cuda:0'))
                # print("Reserved LOSS:", torch.cuda.memory_reserved('cuda:0')) 
  ...
  def forward_backward(self, ids, targets=None, reduction='mean'):
    B, T = ids.size()
    assert T <= self.config.block_size, f"input sequence length {T} exceeds block size {self.config.block_size}"
    # with torch.cuda.amp.autocast(dtype=torch.bfloat16):
    # with torch.amp.autocast('cuda', dtype=torch.bfloat16):
    ...
    if targets is not None:
        ...
        if reduction == 'mean':
            ...
            loss = total_loss   
        return loss  # Return last chunk logits when loss is requested
